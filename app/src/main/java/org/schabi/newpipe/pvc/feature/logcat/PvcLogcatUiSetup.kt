...
object PvcLogcatUiSetup {

    fun setLogcatOutputFileFormat() {
        Settings.Default.update { current ->
            current.copy(
                logfileFormat = PvcLogFileFormat(),
                logFileName = PvcLogFileName(),
 ...
 */
class PvcLogFileFormat : LogFileFormat {
...
open class PvcLogFileName : LogFileName {
    protected val dateFormat =
        SimpleDateFormat("'PVCPipe_${BuildConfig.BUILD_TYPE}_'yyyy-MM-dd_HH-mm-ss-SSS'.log'", Locale.ROOT)
